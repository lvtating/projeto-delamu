...
export default function GeradorPage() {
  ...
  useEffect(() => {
    const carregarTemplates = async () => {
      try {
        ...
        if (!resFrente.ok || !resVerso.ok) throw new Error("Templates não encontrados");
      ...
      try {
        ...
        if (indexLinhaCrono !== -1) {
          ...
          for (let i = indexLinhaCrono + 1; i < matriz.length; i++) {
            ...
            const obj: any = {};
            cabecalhoCrono.forEach((label, idx) => { if(label) obj[label] = linha[idx]; });
            listaCrono.push(obj);
          }
        }
        setDados({ alunos: listaAlunos, cronograma: listaCrono });
        alert(`${listaAlunos.length} alunos e ${listaCrono.length} aulas carregadas com sucesso!`);      } catch (err) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault(); 
    if (dados && templateFrente && templateVerso) {
      gerarZipCertificados(dados.alunos, dados.cronograma, templateFrente, templateVerso, gestao);
    }
  };
  ...
  return (
    <main className="w-full min-h-screen flex flex-col items-center p-8 bg-no-repeat bg-bottom bg-cover" style={{ backgroundImage: 'url(/background.svg)' }}>
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-md space-y-6">
        ...
        {/* primeiro container (seção de assinaturas)*/}
        <div className="bg-[#f4f1e9]/60 p-6 rounded-lg shadow-md space-y-4 border border-gray-100">
          <h2 className="font-bold text-[#a62828] border-b pb-2 flex items-center gap-2">
            <span>✍️</span> Assinaturas do Certificado
          </h2>
          
          <div>
            <label htmlFor="coordenadorLiga" className="block text-xs font-semibold text-[#1a1817] uppercase">Coordenador da Liga</label>
            <input 
              type="text" 
              id="coordenadorLiga"
              name="coordenadorLiga"
          ...
          <div>
            <label htmlFor="presidenteDelamu" className="block text-xs font-semibold text-[#1a1817] uppercase">Presidente do DeLAMU</label>
            <input 
              type="text" 
              id="presidenteDelamu"
              name="presidenteDelamu"
          ...
          <div>
            <label htmlFor="coordenadorCurso" className="block text-xs font-semibold text-[#1a1817] uppercase">Coordenador do Curso</label>
            <input 
              type="text" 
              id="coordenadorCurso"
        ...
        {/* segundo container (upload e geração)*/}
        <div className="bg-[#f4f1e9]/60 p-6 rounded-lg shadow-md space-y-6 border border-gray-100">
          ...
          <div className="flex items-center justify-between text-xs">
            <span className={templateFrente && templateVerso ? "text-green-600 font-bold" : "text-amber-600"}>
              {templateFrente && templateVerso ? "✅ Templates Prontos" : "⏳ Carregando..."}
            </span>
